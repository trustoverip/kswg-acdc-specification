...
The initial element in each deterministic derivation path is the string value of the bulk-issued member's copy index `k`, such as `0`, `1`, `2`, etc.  Specifically, if `k` denotes the index of an ordered set of bulk-issued private ACDCs of size `M`, the derivation path starts with the string `k`, where `k` is replaced with the textual representation of the numeric index `k`. That representation MUST be lowercase hexadecimal with no leading zeros, so that the index `10` is rendered `a` and the index `255` is rendered `ff`. The same rendering MUST be used for every other numeric index that appears in a derivation path, including the nested block and aggregate element index `j` described below. Furthermore, a bulk-issued private ACDC with a private Attribute section uses `k` to derive its top-level unique entropy and `k/0` to derive its Attribute section unique entropy. This hierarchical path is extended to any nested private Attribute blocks.
...
To make the aggregate `B` reproducible and conformance-testable across independent implementations, both the concatenation domain and the digest operator MUST be pinned. The concatenation operators `+` (above) and `C` (below) operate on the fully qualified CESR (qb64 text) representations of their operands, not on their raw binary values. Concatenating the qb64 text is well-defined regardless of the underlying digest lengths and serialization kind, which is the crypto-agility property the previous paragraph relies on. The digest operator H MUST use the same digest algorithm for every member of a given bulk-issued set and for the aggregate `B`; unless a different algorithm is negotiated out of band, H is Blake3-256, and its output is the fully qualified CESR (qb64) digest primitive. Consequently, each b<sub>k</sub> and the aggregate `B` are CESR digest primitives. Two implementations that use the same salt, template, member count, and digest algorithm therefore compute identical b<sub>k</sub> and `B` values, so a bulk-membership proof produced by one verifies at the other.
...
These three methods apply to independent Registry bulk-issued ACDCs, described below, as well as to the basic case, but the reason changes. With independent Registries, the Registry SAID no longer correlates a copy to the other members of its bulk-issued set, because no two members of the set share a Registry. It remains, however, a stable identifier for the copy itself, and hence for the context in which that copy is used. A Discloser that discloses the Registry SAID before contractual protection is in place therefore leaves a prospective Disclosee holding a durable identifier for that context even if that Disclosee then declines the terms. A Discloser SHOULD NOT disclose the Registry SAID prior to contractual protection regardless of which bulk issuance variant is in use.
...
When each copy of a bulk-issued set gets its own Registry, there are two ways to organize the relationship between Registry creation and ACDC creation, and they make opposite tradeoffs. In the first, called here *pre-created Registry* bulk issuance, the Issuer creates Registries in advance, using Registry inception, `rip`, events that are not associated with any ACDC at the time of their creation and that are associated with one only at some later time. In the second, called here *co-created Registry* bulk issuance, the Issuer creates each copy's Registry at the same time as it creates the bulk-issued set itself, deriving that Registry's `rip` event from the same shared secret salt used to derive that copy's unique-entropy values. An Issuer MUST NOT assume that the choice is immaterial; it determines both what the Issuee must store and which parties are able to correlate.

Pre-created Registries decorrelate the creation of a Registry, and the subsequent updates to its state, from any data in the ACDC that is later applied to it, including that ACDC's datetimes. This decorrelation holds against 3rd parties observing the Issuer's KEL and the Registry, and not merely against colluding 2nd party Verifiers, because no observable property of a `rip` event, nor of the moment it was created, is a function of an ACDC that did not exist when it was created. Registries may accordingly be pre-created in quantity and anchored in a single batch, and the Issuer may then use the state-preserving update events described below so that a batch of updates in which some Registries are assigned to ACDCs is indistinguishable from one in which none are. The cost is that the identity of the Registry belonging to a given copy cannot be regenerated. The Registry SAID, `rd`, is the SAID of the `rip` event, whose required fields include the Issuer's datetime, `dt`, so `rd` is not a function of the shared secret salt under any derivation path whatsoever.

Consequently, when Registries are pre-created, the bulk issuance process MUST be augmented to convey out of band what cannot be derived. The Issuer MUST provide the Issuee with a list, indexed by the bulk-issued set member index `k`, whose entry at index `k` gives the Registry SAID, `rd`, of the Registry assigned to copy `k`. The Issuee MUST retain this list. It is not recoverable from the template and the shared secret salt, and without it the Issuee cannot populate the Registry SAID, `rd`, field of copy `k` when it generates that copy. Nothing else need be conveyed: the blinding salt of copy `k`'s Registry is derived, as described below, from the same shared secret salt as the ACDCs themselves.

The Blinded State Disclosure section above requires that the blind of a blindable state update event derive from a secret salt shared between the Issuer and the designated Discloser, and says nothing about how that salt relates to the shared secret salt of a bulk-issued set. Read alone, it invites the conclusion that every Registry carries a salt of its own, which an Issuer would then have to convey. That is one permissible arrangement, but it is not the one specified here, because it obliges an Issuer to convey `M` secrets where one suffices. In independent Registry bulk issuance, the blinding salt of copy `k`'s Registry MUST instead be derived from the shared secret salt of the bulk-issued set, at the path `k.s`, that is, `k` followed by a period (`.`) followed by the letter `s`, where `k` is the textual representation of the copy index, rendered as lowercase hexadecimal with no leading zeros as required by the Basic Bulk Issuance Procedure above. Only the one shared secret salt is exchanged, whether the Registries were pre-created or co-created.

That derivation is distinct from, and prior to, the derivation of any individual blind. The value derived at `k.s` is a salt, from which the blind of each blindable state update event of copy `k`'s Registry is then separately derived using that event's sequence number as the path, exactly as for any other blindable state Registry. An implementation MUST NOT derive a blind directly from the shared secret salt of the bulk-issued set, because the path for a blind is the bare sequence number, which is the same namespace that the top-level path `k` occupies, and the two would collide. A single derivation over a compound path that combines the copy index with the sequence number is an equivalent construction in principle, but the two-stage form is specified here because it composes with an implementation of blindable state that is given a salt and a sequence number and knows nothing of bulk issuance.

Where a Registry was pre-created, this derivation governs its blindable state update events from the moment the Registry is assigned to copy `k` onward. Any blind of a placeholder event published before that assignment is the Issuer's own affair, since such an event carries an empty transaction event ACDC SAID, `td`, and an empty transaction state, `ts`, and need not be recoverable by the Issuee.

Co-created Registries preserve regenerability at the cost of that decorrelation. Because each copy's Registry is incepted in the same moment as the set itself, the moment of Registry creation correlates with the datetimes carried in the ACDCs of the set, which is a point of correlation available to any 3rd party observing the Issuer's KEL. When, and only when, Registries are co-created, one further value is derived per copy: the unique-entropy, `u`, field of the Registry inception event, `rip`, for copy `k` MUST be derived at the path `k.r`, that is, `k` followed by a period (`.`) followed by the letter `r`.

Like the blinding factor path `k.`, the paths `k.s` and `k.r` by construction never collide with the top-level path `k`, the Attribute section path `k/0`, or any nested block or aggregate element path `k/j`. The bare path `k.` is not available for either of them, because the modular extension described below retains a per-set aggregate `B` and hence the blinding factors v<sub>k</sub> that `k.` derives. The suffix following the period is what distinguishes them from one another, and those suffixes are drawn from a reserved namespace: the empty suffix, which denotes the aggregate blinding factor v<sub>k</sub> of the Basic Bulk Issuance Procedure, and otherwise one or more characters drawn from the letters `g` through `z`, being those letters that are not also hexadecimal digits. No suffix drawn from that namespace can be confused with a numeric sub-index rendered as lowercase hexadecimal as required above. A suffix of `b`, or of a digit such as `0`, collides with nothing in the namespace as it stands today, but would become ambiguous were a numeric sub-index ever admitted after the period, and is for that reason not used here.

Deriving the unique entropy of a `rip` event is necessary but not by itself sufficient to regenerate that Registry's SAID, because the SAID is a digest over the whole event. For co-created Registries the template that the Issuer provides to the Issuee MUST therefore include a `rip` event template alongside the ACDC template, in which every field other than the unique entropy, `u`, and the SAID, `d`, holds the value it holds in every Registry of the set, the Issuer's datetime, `dt`, in particular. Given that template and the shared secret salt, the Issuee derives u<sub>k</sub> at the path `k.r`, completes the `rip` event, computes its SAID, and uses that SAID as the Registry SAID, `rd`, field value of copy `k`. It bears noting that the single datetime shared across the whole set, which is what makes `rd` regenerable, is the same fact that correlates the members of the set to one another and to the moment of issuance. The two properties are not independently obtainable.

Deriving the blinding salt per copy is what bounds a Disclosee's reach, and is the reason the derivation is at `k.s` rather than at a path shared by the set. A Disclosee given the blinding salt of copy `k` may unblind the state of copy `k`'s Registry and no other, whereas a blinding salt shared across the set would enable that Disclosee to unblind every state update of every member of the set, reintroducing as a 2nd party correlator precisely what independent Registries remove. The hazard is sharper than it may appear, because the derivation path for a blind within a Registry is the sequence number of the transaction event alone, so two Registries that share a blinding salt do not merely admit unblinding by a common holder of that salt but produce the identical blind at the same sequence number. An Issuer may nonetheless have a countervailing reason to share one blinding salt across a group of Registries, such as enabling a Disclosee to validate a chain of authority whose intermediate blinded states it has no other means to obtain. That is outside independent Registry bulk issuance, whose whole purpose is the decorrelation such sharing gives up, and an Issuer that shares a blinding salt across a group accepts provable correlation within that group.

Independent Registry bulk issuance does not use the blinded aggregate, `B`, and consequently does not use the blinding factors, v<sub>k</sub>, from which `B` is computed. The aggregate exists to enable a set of copies to share one TEL without that TEL making a commitment to any individual member of the set. When each copy has its own TEL, the state update event for copy `k` makes its commitment to that copy's SAID, d<sub>k</sub>, directly, and there is nothing left to aggregate. Accordingly, when using a blindable state update, `bup`, event, the blinded state for copy `k` MUST bind d<sub>k</sub> as its transaction event ACDC SAID, `td`, field value. The `td = B` construction of the Basic Bulk Issuance Procedure MUST NOT be used for independent Registry bulk issuance. The modular extension described next is an intermediate case: each set in the set of sets retains its own aggregate `B`, because each Registry there is still shared by the members of one set.
...
A Discloser may make a provable non-repudiable disclosure of a member of an independent Registry bulk-issued set, at index `k`, by providing to the Disclosee four items of information. These are as follows:

1. The ACDC in compact form (at index `k`) where d<sub>k</sub> is the value of its top-level SAID, `d`, field.
2. The Registry inception event, `rip`, for copy `k`, whose SAID is the value of that ACDC's Registry SAID, `rd`, field.
3. The applicable state update event from that Registry, together with the blinding salt for copy `k` when that event is a blindable state update, `bup`, event.
4. Either a reference to or a disclosure of the anchoring seal in the Issuer's KEL that makes a commitment to that state update event, together with any inclusion proof required to verify that commitment, as described in the section below.

A Disclosee may then verify the disclosure by:

1. Computing d<sub>k</sub> on the disclosed compact ACDC and confirming that the ACDC's Registry SAID, `rd`, field value is the SAID of the disclosed Registry inception event, `rip`.
2. Confirming that the Issuer identifier, `i`, field value of the disclosed `rip` event is the same as the Issuer identifier, `i`, field value of the ACDC.
3. Unblinding the state update event, when it is blinded, using the disclosed blinding salt, and confirming that the resulting transaction event ACDC SAID, `td`, field value is d<sub>k</sub>.
4. Confirming that the anchoring seal in the Issuer's KEL makes a commitment to that state update event, and verifying the Key State of the Issuer at the time of the update. This may require additional verification steps as per the KERI protocol.

Step 2 is not specific to bulk issuance, but it is essential to it. Because the Registry SAID, `rd`, field is a secure discovery mechanism, a Verifier MUST confirm that the Registry discovered through `rd` was incepted by the ACDC's own Issuer. A Verifier that omits this step may be directed by a malicious Discloser to a Registry that the Discloser itself controls and whose state it therefore keeps favorable, in which case the state proof establishes nothing about the Issuer's intent. With a single shared Registry per set, an implementation may check the Registry once and cache the result; with independent Registries, a distinct Registry is discovered on every presentation, so the check recurs every time and is correspondingly easier to omit.
...
The Merkle tree needs to have appropriate second-pre-image attack protection of interior branch nodes [[50](#MTSec)] [[51](#ref51)]. The Discloser then only needs to provide a subset of digests from the Merkle tree to prove that a given transaction event SAID contributed to the Merkle tree root digest.

Two different structures appear in this section, and a bare Merkle root seal does not distinguish them. The paragraphs above describe a single amalgamated sparse Merkle tree spanning every transaction event of every Registry of the Issuer, whose current root the Issuer anchors periodically and whose incremental updates the Registrar publishes to Observers. A sparse Merkle tree is indexed by key, assigns a default value to absent leaves, and yields inclusion proofs of fixed depth. A dense Merkle tree over the transaction events of a single batch is a different structure, whose leaves are only the events of that batch and whose proof length varies with the size of the batch. Both provide herd privacy and inclusion proofs that reveal no other transaction event, but they are not interchangeable, because an inclusion proof against one does not verify against the root of the other, and a Validator presented with a root digest alone has no way to tell which it holds.

The KERI specification anticipates this. Its Merkle tree root digest seal carries only the root, `rd`, whereas its typed seal carries a versioned type, `t`, alongside the digest, `d`, for the stated reason that the construction of a tree and its proofs of inclusion may differ by type, such as a sparse Merkle tree versus a non-sparse one. Accordingly, an Issuer anchoring a batch of independent Registry transaction events SHOULD use a typed seal rather than a bare Merkle tree root digest seal, so that the tree type and version a Validator must assume are committed in band by the Issuer rather than agreed out of band. An Issuer that uses a bare Merkle tree root digest seal MUST establish the tree type out of band with every party expected to verify an inclusion proof against it.

So that a batch root computed by one implementation verifies at another, the construction of a dense batch tree MUST be as follows unless a different type is indicated by the anchoring seal or negotiated out of band. A sparse Merkle tree requires a pinned construction of its own, which this specification does not yet provide.

1. Each leaf of the tree corresponds to one transaction event, and the value committed by that leaf is the SAID of that transaction event. The digest of a leaf is given by H(0x00 + t), where `t` is the transaction event SAID in the CESR qb64 text domain, `+` is the infix concatenation operator, and 0x00 is a single prefixed octet.
2. The digest of an interior node is given by H(0x01 + l + r), where `l` and `r` are the digests of its left and right children respectively, and 0x01 is a single prefixed octet.
3. The distinct 0x00 and 0x01 domain separation prefixes provide the second-pre-image protection required above, by ensuring that no interior node digest may be reinterpreted as a leaf digest.
4. When a level of the tree contains an odd number of nodes, the unpaired node MUST be promoted unchanged to the next level. It MUST NOT be duplicated and paired with itself, because duplication admits distinct sequences of leaves that produce the same root digest.
5. The digest operator `H` is Blake3-256 with CESR qb64 output by default, matching the digest pinned for the constituents of the aggregate `B` in the Basic Bulk Issuance Procedure.

One anchoring seal covers every transaction event in the batch. Where that seal is a typed seal, its digest, `d`, field is the root digest of the tree and its type, `t`, field indicates the construction above. Where it is a Merkle tree root digest seal, its root, `rd`, field is the root digest and the construction is not committed in band.

An inclusion proof for a transaction event consists of the ordered sibling digests from the leaf level to the root, together with, for each, an indication of whether that sibling is the left or the right child. A Validator recomputes the leaf digest from the transaction event SAID, folds in each sibling digest on the indicated side using the interior node construction above, and confirms that the resulting digest is the root digest committed by the Issuer's anchoring seal. Such a proof discloses interior node digests only, and therefore reveals no other transaction event SAID.

The Registries of one bulk-issued set SHOULD NOT advance in lock step. Nothing requires the sequence numbers of the Registries belonging to one set to track one another, and an Issuer SHOULD ensure that they do not, by issuing differing numbers of state-preserving update events to each. Two Registries whose event counts and update times move together are correlatable to a 3rd party by that fact alone, whatever their state, which defeats the purpose for which independent Registries were adopted. Because the blind of each event derives from that Registry's own sequence number, divergent sequence numbers follow naturally from per-Registry blinding salts and require no additional derivation machinery.

An Issuer SHOULD ensure that a given batch mixes transaction events across Registries and across Issuees. A batch that contains only the events of a single Issuee provides no herd privacy: its root, and the fact that its events were co-anchored, correlate precisely the events that independent Registries were adopted to decorrelate. The state-preserving update events described above are one means of ensuring an adequate mixture. For the same reason, an Issuer SHOULD NOT anchor the Registry inception events of a bulk-issued set as a list of individual seals in a single key event, because doing so relocates the correlation from the Registries to the Issuer's KEL rather than removing it.
