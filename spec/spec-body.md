...
Like the blinding factor path `k.`, these paths by construction never collide with the top-level path `k`, the Attribute section path `k/0`, or any nested block or aggregate element path `k/j`. The characters that follow the period are furthermore drawn from those letters that are not also hexadecimal digits, that is from `g` through `z`, so that no suffix can ever be confused with a numeric sub-index rendered as lowercase hexadecimal as required above. A suffix of `b`, or of a digit such as `0`, would satisfy the first constraint but not the second, and is for that reason not used here.
