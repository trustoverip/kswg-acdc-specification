...
When each copy of a bulk-issued set gets its own Registry, the Registry inception event, `rip`, for each copy is generated on the fly from the same shared secret salt used to derive that copy's UUIDs, so that the Issuee continues to store only the template and the salt rather than a copy of each Registry. Two additional values are therefore derived per copy, and their derivation paths MUST be as follows, where `k` is the decimal or hexadecimal textual representation of the copy index as defined in the Basic Bulk Issuance Procedure above:
