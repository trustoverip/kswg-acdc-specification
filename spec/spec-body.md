...
When each copy of a bulk-issued set gets its own Registry, the Registry inception event, `rip`, for each copy is generated on the fly from the same shared secret salt used to derive that copy's UUIDs, so that the Issuee continues to store only the template and the salt rather than a copy of each Registry. Two additional values are therefore derived per copy, and their derivation paths MUST be as follows, where `k` is the textual representation of the copy index, rendered as lowercase hexadecimal with no leading zeros as required by the Basic Bulk Issuance Procedure above:
...
2. When that Registry uses blindable state update, `bup`, events, the blinding salt for copy `k`'s Registry MUST be derived at the path `k.b`, that is, `k` followed by a period (`.`) followed by the letter `b`.

Like the blinding factor path `k.`, these paths by construction never collide with the top-level path `k`, the Attribute section path `k/0`, or any nested block or aggregate element path `k/j`. Deriving the Registry blinding salt per copy, rather than sharing one blinding salt across the set, bounds what a given Disclosee is able to unblind. A Disclosee given the blinding salt for copy `k` may unblind the state of copy `k`'s Registry and no other, whereas a blinding salt shared across the set would enable that Disclosee to unblind every state update of every member of the set, which would reintroduce as a 2nd party correlator precisely what independent Registries remove.
...
So that a batch root computed by one implementation verifies at another, the construction of the tree MUST be as follows unless negotiated out of band.
...
The anchoring seal that commits a batch root is the Merkle root seal, whose field, `rd`, is the root digest of the tree. One such seal covers every transaction event in the batch.
