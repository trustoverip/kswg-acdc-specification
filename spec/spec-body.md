...
The initial element in each deterministic derivation path is the string value of the bulk-issued member's copy index `k`, such as `0`, `1`, `2`, etc.  Specifically, if `k` denotes the index of an ordered set of bulk-issued private ACDCs of size `M`, the derivation path starts with the string `k`, where `k` is replaced with the textual representation of the numeric index `k`. That representation MUST be lowercase hexadecimal with no leading zeros, so that the index `10` is rendered `a` and the index `255` is rendered `ff`. The same rendering MUST be used for every other numeric index that appears in a derivation path, including the nested block and aggregate element index `j` described below. A hexadecimal and a decimal rendering of the same index agree for indices 0 through 9 and diverge from index 10 onward, so leaving the choice open would mean that two otherwise conformant implementations derive different unique-entropy values, and hence different SAIDs, for every member of a bulk-issued set from the tenth onward, while agreeing on the first ten. This is precisely the failure mode that is expensive to detect, because worked examples and test vectors are usually small enough to fall entirely within the range where the two renderings coincide. Lowercase hexadecimal with no leading zeros is also what is already used for every other hierarchical deterministic derivation path in KERI and ACDC, including the sequence number used as the derivation path for the blind of a blindable state update event. Furthermore, a bulk-issued private ACDC with a private Attribute section uses `k` to derive its top-level unique entropy and `k/0` to derive its Attribute section unique entropy. This hierarchical path is extended to any nested private Attribute blocks.
...
Consequently, when Registries are pre-created, the bulk issuance process MUST be augmented to convey out of band what cannot be derived. The Issuer MUST provide the Issuee with a list, indexed by the bulk-issued set member index `k`, whose entry at index `k` gives the Registry SAID, `rd`, of the Registry assigned to copy `k` and, when that Registry uses blindable state update, `bup`, events, the blinding salt for that Registry. The Issuee MUST retain this list. It is not recoverable from the template and the shared secret salt, and without it the Issuee can neither populate the Registry SAID, `rd`, field of copy `k` when it generates that copy nor unblind that Registry's state at the time of disclosure. The blinding salt of a pre-created Registry is generated per Registry when that Registry is incepted, independently of any bulk-issued set index, because at that time no such index exists.

Co-created Registries preserve regenerability at the cost of that decorrelation. Because each copy's Registry is incepted in the same moment as the set itself, the moment of Registry creation correlates with the datetimes carried in the ACDCs of the set, which is a point of correlation available to any 3rd party observing the Issuer's KEL. When, and only when, Registries are co-created, two additional values are derived per copy, and their derivation paths MUST be as follows, where `k` is the textual representation of the copy index, rendered as lowercase hexadecimal with no leading zeros as required by the Basic Bulk Issuance Procedure above:

1. The UUID, `u`, field of the Registry inception event, `rip`, for copy `k` MUST be derived at the path `k.r`, that is, `k` followed by a period (`.`) followed by the letter `r`.

2. When that Registry uses blindable state update, `bup`, events, the blinding salt for copy `k`'s Registry MUST be derived at the path `k.s`, that is, `k` followed by a period (`.`) followed by the letter `s`.

These two derivations are distinct from, and prior to, the derivation of any individual blind. The value derived at `k.s` is a salt, from which the blind of each blindable state update event of copy `k`'s Registry is then separately derived using that event's sequence number as the path, exactly as for any other blindable state Registry. An implementation MUST NOT collapse the two stages by deriving a blind directly from the shared secret salt, because the path for a blind is the bare sequence number, which is the same namespace that the top-level path `k` occupies, and the two would collide.

Like the blinding factor path `k.`, these paths by construction never collide with the top-level path `k`, the Attribute section path `k/0`, or any nested block or aggregate element path `k/j`. The suffix following the period is what distinguishes them from one another, and those suffixes are drawn from a reserved namespace: the empty suffix, which denotes the aggregate blinding factor v<sub>k</sub> of the Basic Bulk Issuance Procedure, and otherwise one or more characters drawn from the letters `g` through `z`, being those letters that are not also hexadecimal digits. No suffix drawn from that namespace can be confused with a numeric sub-index rendered as lowercase hexadecimal as required above. A suffix of `b`, or of a digit such as `0`, collides with nothing in the namespace as it stands today, but would become ambiguous were a numeric sub-index ever admitted after the period, and is for that reason not used here.

Deriving the UUID of a `rip` event is necessary but not by itself sufficient to regenerate that Registry's SAID, because the SAID is a digest over the whole event. For co-created Registries the template that the Issuer provides to the Issuee MUST therefore include a `rip` event template alongside the ACDC template, in which every field other than the UUID, `u`, and the SAID, `d`, holds the value it holds in every Registry of the set, the Issuer's datetime, `dt`, in particular. Given that template and the shared secret salt, the Issuee derives u<sub>k</sub> at the path `k.r`, completes the `rip` event, computes its SAID, and uses that SAID as the Registry SAID, `rd`, field value of copy `k`. It bears noting that the single datetime shared across the whole set, which is what makes `rd` regenerable, is the same fact that correlates the members of the set to one another and to the moment of issuance. The two properties are not independently obtainable.

In both cases, the blinding salt of a Registry SHOULD be scoped to a single copy rather than shared across the set, which bounds what a given Disclosee is able to unblind. A Disclosee given the blinding salt of copy `k` may unblind the state of copy `k`'s Registry and no other, whereas a blinding salt shared across the set would enable that Disclosee to unblind every state update of every member of the set, reintroducing as a 2nd party correlator precisely what independent Registries remove. The hazard is sharper than it may appear, because the derivation path for a blind within a Registry is the sequence number of the transaction event alone, so two Registries that share a blinding salt do not merely admit unblinding by a common holder of that salt but produce the identical blind at the same sequence number. This is a recommendation rather than a requirement because an Issuer may have a countervailing reason to share one blinding salt across a group of Registries, such as enabling a Disclosee to validate a chain of authority whose intermediate blinded states it has no other means to obtain. An Issuer that shares a blinding salt across a group accepts provable correlation within that group.
